...
parser.add_argument('--checkpoint_dir', dest='checkpoint_dir', default='\\'.join(os.path.realpath(__file__).split('\\')[:-1])+'\\checkpoint',
                    help='models are saved here')
parser.add_argument('--summaries_dir', dest='summaries_dir', default='\\'.join(os.path.realpath(__file__).split('\\')[:-1])+'\\summary',
                    help='sample are saved here')
parser.add_argument('--test_out_dir', dest='test_out_dir', default='\\'.join(os.path.realpath(__file__).split('\\')[:-1])+'\\result',
                    help='test sample are saved here')
parser.add_argument('--test_dir', dest='test_dir', default='\\'.join(os.path.realpath(__file__).split('\\')[:-1])+'\\test',
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.mode == 'train':
            ...
            model.freeze_save_graph(sess=sess,  output_node='contours', path='./', name='test.pb')
