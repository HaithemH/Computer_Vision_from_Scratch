...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        batch_segms = torch.zeros(
            self.batch_per_gpu, batch_height , batch_width ).long()
        ...
        for i in range(self.batch_per_gpu):
            ...
            segm = imresize(segm,(segm.size[0], segm.size[1]), interp='nearest')
            ...
            # return img.shape, segm[:, :, -1].shape, batch_images.shape, batch_segms.shape

            # put into batch arrays
            batch_images[i][:, :img.shape[1], :img.shape[2]] = img
            batch_segms[i][:segm.shape[0], :segm.shape[1]] = segm[:, :, -1]
